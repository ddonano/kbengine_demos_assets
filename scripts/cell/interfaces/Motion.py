...
class Motion:
    ...
    def randomWalk(self, basePos):
        ...
        while True:
            # 移动半径距离在30米内
            if self.canNavigate():
                destPos = self.getRandomPoints(basePos, self.__TERRITORY_AREA__, 1, 0)
                ...
                rnd = random.random()
                a = self.__TERRITORY_AREA__ * rnd  # 移动半径距离在30米内
                b = 360.0 * rnd  # 随机一个角度
    ...
    def changeMoveSpeed(self, exposed, moveSpeed):
        # self.speed = moveSpeed
        if exposed != self.id:
            return
        self.moveSpeed = moveSpeed
        INFO_MSG("%s::changeMoveSpeed: %i, speed=%f." % \
                 (self.getScriptName(), self.id, self.moveSpeed * 0.1))

    # self.allClients.onChangeMoveSpeed(moveSpeed)
    ...
    def gotoPosition(self, position, dist=0.0):
        ...
        if self.canNavigate():
            ...
            self.moveToPoint(destPos, speed, 0.0, 0, True, True)
